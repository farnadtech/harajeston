@auth
@if(!auth()->user()->isVerified() && auth()->user()->role !== 'admin')
<div class="bg-amber-50 border border-amber-300 rounded-2xl p-5 mb-6"
     x-data="{
        showEmailForm: {{ session('email_otp_sent') ? 'true' : 'false' }},
        showPhoneForm: false,
        phoneStep: 'send',
        ...
        async sendPhoneOtp() {
            ...
            this.phoneMsg = '';
            try {
                const res = await fetch('{{ route('phone.verify.send') }}', {
                    method: 'POST',
                    headers: {
    ...
    <div class="flex items-start gap-4">
        ...
        <div class="flex-1">
            ...
            @if(session('success') && !session('email_otp_sent'))
                <div class="bg-green-50 border border-green-200 text-green-700 px-3 py-2 rounded-lg text-sm mb-3">{{ session('success') }}</div>
            @endif
            @if(session('error'))
                <div class="bg-red-50 border border-red-200 text-red-700 px-3 py-2 rounded-lg text-sm mb-3">{{ session('error') }}</div>
            @endif
            ...
            <div x-show="!showEmailForm && !showPhoneForm" style="display:flex" class="flex flex-wrap gap-3">
                ...
                @if(!auth()->user()->email_verified_at)
                    <form action="{{ route('email.verify.send') }}" method="POST" class="inline" @submit.prevent="showEmailForm = true; $el.submit()">
                        @csrf
                        <button type="submit"
            ...
            {{-- فرم تایید شماره تلفن (popup inline) --}}
            <div x-show="showPhoneForm" style="display:none" class="bg-white border border-gray-200 rounded-xl p-4 mt-2 max-w-sm">
                <p class="text-sm font-medium text-gray-700 mb-3">تایید شماره: <span class="font-bold text-primary" dir="ltr">{{ auth()->user()->phone }}</span></p>
            ...
            <div x-show="showEmailForm" style="display:none" class="bg-white border border-gray-200 rounded-xl p-4 mt-2 max-w-sm">
                ...
                @endif
                @if($errors->has('email_otp_code'))
                    <p class="text-sm text-red-600 mb-2">{{ $errors->first('email_otp_code') }}</p>
                @endif
                <form action="{{ route('email.verify') }}" method="POST" class="space-y-3">
                    @csrf
                    <input type="text" name="email_otp_code" maxlength="6" placeholder="کد ۶ رقمی"
                        class="w-full border border-gray-300 rounded-xl px-4 py-2 text-sm focus:ring-2 focus:ring-primary text-center tracking-widest"
                        dir="ltr" inputmode="numeric" value="{{ old('email_otp_code') }}">
                ...
                <form action="{{ route('email.verify.send') }}" method="POST" class="mt-2">
                    @csrf
