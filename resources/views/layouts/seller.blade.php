                            ...
                            <div class="flex items-center justify-between">
                                ...
                                @if(auth()->user()->notifications()->where('is_read', false)->count() > 0)
                                    <span class="text-xs bg-red-100 text-red-600 px-2 py-1 rounded-full font-medium">
                                        @persian(auth()->user()->notifications()->where('is_read', false)->count()) جدید
                                    </span>
                                @endif
                            </div>
                        </div>
                                ...
                                <div class="block p-4 hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-0">
                                    <div class="flex items-start gap-3">
                                        <div class="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center shrink-0">
                                            <span class="material-symbols-outlined text-blue-600 text-sm">{{ $notification->icon ?? 'notifications' }}</span>
                                        </div>
                                        <div class="flex-1 min-w-0">
                                            <p class="text-sm text-gray-800 {{ $notification->is_read ? '' : 'font-bold' }}">
                                                {{ $notification->message ?? 'اعلان جدید' }}
                                            </p>
