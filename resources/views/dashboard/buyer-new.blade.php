...
<body class="bg-background-light text-[#0d121b] antialiased min-h-screen flex overflow-hidden">
    ...
    <aside class="w-64 bg-white border-l border-gray-200 hidden lg:flex flex-col h-screen fixed right-0 top-0 z-30">
        ...
        <nav class="flex-1 overflow-y-auto py-6 px-4 space-y-1">
            ...
            <a class="flex items-center gap-3 px-4 py-3 text-gray-600 hover:text-primary hover:bg-gray-50 rounded-xl font-medium transition-colors group" href="{{ route('my-bids') }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div class="bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-2xl p-6 shadow-lg">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span class="text-3xl font-black">{{ \App\Services\PersianNumberService::convertToPersian($activeBidsCount ?? 0) }}</span>
                    </div>
                ...
                <div class="bg-gradient-to-br from-green-500 to-emerald-600 text-white rounded-2xl p-6 shadow-lg">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span class="text-3xl font-black">{{ \App\Services\PersianNumberService::convertToPersian($wonAuctionsCount ?? 0) }}</span>
                    </div>
                ...
                <div class="bg-gradient-to-br from-purple-500 to-purple-600 text-white rounded-2xl p-6 shadow-lg">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span class="text-3xl font-black">{{ \App\Services\PersianNumberService::convertToPersian($totalOrdersCount ?? 0) }}</span>
                    </div>
                ...
                <div class="bg-gradient-to-br from-amber-500 to-orange-600 text-white rounded-2xl p-6 shadow-lg">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span class="text-2xl font-black">{{ \App\Services\PersianNumberService::convertToPersian(number_format(auth()->user()->wallet->balance ?? 0)) }}</span>
                    </div>
                ...
                <div class="lg:col-span-2 space-y-8">
                    ...
                    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                        <div class="p-6 border-b border-gray-100 flex items-center justify-between">
                            ...
                            <a href="{{ route('listings.index') }}" class="text-sm font-medium text-primary hover:text-blue-700 flex items-center gap-1">
                                <span>مشاهده همه</span>
                                <span class="material-symbols-outlined text-lg">arrow_back</span>
                            </a>
                        </div>
                        <div class="p-6">
                            @forelse($myActiveBids ?? [] as $bid)
                                <div class="flex items-center gap-4 p-4 hover:bg-gray-50 rounded-xl transition-colors mb-4 last:mb-0">
                                    <img src="{{ $bid->listing->images->first()?->url ?? '/images/placeholder.jpg' }}" 
                                         alt="{{ $bid->listing->title }}" 
                                         class="w-20 h-20 object-cover rounded-lg">
                                    <div class="flex-1 min-w-0">
                                        <h4 class="font-bold text-gray-900 truncate">{{ $bid->listing->title }}</h4>
                                        <p class="text-sm text-gray-500 mt-1">پیشنهاد شما: {{ \App\Services\PersianNumberService::convertToPersian(number_format($bid->amount)) }} تومان</p>
                                        <p class="text-xs text-gray-400 mt-1">{{ $bid->created_at->diffForHumans() }}</p>
                                    </div>
                                    <a href="{{ route('listings.show', $bid->listing) }}" class="px-4 py-2 bg-primary text-white rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium">
                                        مشاهده
                                    </a>
                                </div>
                            @empty
                                <div class="text-center py-12">
                                    ...
                                    <a href="{{ route('listings.index') }}" class="inline-block mt-4 px-6 py-2 bg-primary text-white rounded-lg hover:bg-blue-700 transition-colors">
                                        مشاهده مزایده‌ها
                                    </a>
                                </div>
                            @endforelse
                    ...
                    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                        <div class="p-6 border-b border-gray-100 flex items-center justify-between">
                            ...
                            <a href="{{ route('orders.index') }}" class="text-sm font-medium text-primary hover:text-blue-700 flex items-center gap-1">
                                <span>مشاهده همه</span>
                                <span class="material-symbols-outlined text-lg">arrow_back</span>
                            </a>
                        </div>
                        <div class="p-6">
                            @forelse($recentOrders ?? [] as $order)
                                <div class="flex items-center gap-4 p-4 hover:bg-gray-50 rounded-xl transition-colors mb-4 last:mb-0">
                                    <div class="w-12 h-12 rounded-full bg-{{ $order->status === 'completed' ? 'green' : ($order->status === 'pending' ? 'yellow' : 'gray') }}-100 flex items-center justify-center">
                                        <span class="material-symbols-outlined text-{{ $order->status === 'completed' ? 'green' : ($order->status === 'pending' ? 'yellow' : 'gray') }}-600">shopping_bag</span>
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <h4 class="font-bold text-gray-900">سفارش #{{ \App\Services\PersianNumberService::convertToPersian($order->id) }}</h4>
                                        <p class="text-sm text-gray-500 mt-1">{{ \App\Services\PersianNumberService::convertToPersian(number_format($order->total_amount)) }} تومان</p>
                                        <p class="text-xs text-gray-400 mt-1">{{ $order->created_at->diffForHumans() }}</p>
                                    </div>
                                    <a href="{{ route('orders.show', $order) }}" class="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors text-sm font-medium">
                                        جزئیات
                                    </a>
                                </div>
                            @empty
