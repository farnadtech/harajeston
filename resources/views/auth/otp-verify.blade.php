    ...
    <div class="bg-white rounded-3xl shadow-2xl overflow-hidden w-full max-w-md p-8 sm:p-12">
        ...
        <div class="mb-8">
            ...
            <p class="text-sm text-gray-500 text-center">
                کد ۶ رقمی به شماره
                <span class="font-bold text-gray-700 mx-1" dir="ltr">{{ $phone }}</span>
                ارسال شد
            </p>
        </div>
        ...
        <form action="{{ route('otp.login.verify') }}" method="POST" class="space-y-5">
            @csrf
        ...
        {{-- فرم ارسال مجدد — خارج از فرم اصلی --}}
        <form id="resend-form" action="{{ route('otp.resend') }}" method="POST" class="hidden">
            @csrf
            <input type="hidden" name="phone" value="{{ $phone }}">
            <input type="hidden" name="purpose" value="{{ $purpose ?? 'login' }}">
        </form>

        <div class="mt-6 text-center">
            <a href="{{ $purpose === 'login' ? route('otp.login') : route('register') }}"
