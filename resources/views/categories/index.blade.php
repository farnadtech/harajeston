...
<div class="w-full max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="space-y-8 pb-12">
        ...
        <div class="bg-white rounded-2xl border overflow-hidden shadow-sm hover:shadow-md transition-shadow"
             style="border-color: {{ $p['border'] }}40;">

            {{-- هدر دسته اصلی --}}
            <a href="{{ route('listings.index', ['category' => $cat->slug]) }}"
               class="flex items-center gap-5 p-6 group transition-all"
               style="background: linear-gradient(135deg, {{ $p['from'] }} 0%, {{ $p['to'] }} 100%); border-bottom: 2px solid {{ $p['border'] }};">
                <div class="w-16 h-16 rounded-2xl flex items-center justify-center flex-shrink-0 shadow-md transition-transform group-hover:scale-110 group-hover:rotate-3"
                     style="background: linear-gradient(135deg, {{ $p['icon'] }}25, {{ $p['icon'] }}10); margin-left: 1rem;">
                    <span class="material-symbols-outlined text-3xl" style="color: {{ $p['icon'] }}">
                        {{ $cat->icon ?? 'category' }}
                    </span>
                </div>
                <div class="flex-1 min-w-0">
                    ...
                    @if($cat->description)
                        <p class="text-gray-500 text-sm mt-1 line-clamp-1">{{ $cat->description }}</p>
                    @endif
                    <div class="flex items-center gap-3 mt-2 flex-wrap">
                        <span style="display:inline-flex; align-items:center; gap:5px; font-size:12px; font-weight:600; padding:4px 12px; border-radius:999px; background-color: {{ $p['icon'] }}20; color: {{ $p['icon'] }}; white-space:nowrap;">
                        ...
                        @if($cat->children->count() > 0)
                            <span style="display:inline-flex; align-items:center; gap:5px; font-size:12px; color:#6b7280; padding:4px 12px; border-radius:999px; background:rgba(255,255,255,.7); white-space:nowrap;">
                                ...
                                {{ $cat->children->count() }} زیردسته
                            </span>
                        @endif
                    </div>
                </div>
                <div class="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center transition-all group-hover:translate-x-1"
                     style="background-color: {{ $p['icon'] }}15;">
                    <span class="material-symbols-outlined rtl:rotate-180" style="color: {{ $p['icon'] }}">arrow_right_alt</span>
                </div>
            </a>
            ...
            <div class="p-6 pt-5">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    ...
                    <div class="rounded-xl border overflow-hidden transition-all hover:shadow-md"
                         style="border-color: {{ $p['border'] }}60; background: {{ $p['from'] }}40;">
                        ...
                        <a href="{{ route('listings.index', ['category' => $child->slug]) }}"
                           ...
                           style="border-bottom: 1px solid {{ $p['border'] }}60;">
                            <div class="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0"
                                 style="background-color: {{ $p['icon'] }}20;">
                                <span class="material-symbols-outlined text-lg" style="color: {{ $p['icon'] }}">
                                    {{ $child->icon ?? 'folder' }}
                                </span>
                            </div>
                        ...
                        {{-- زیردسته‌های سطح ۳ - همه نشون داده میشن --}}
                        ...
                        <div class="p-3 space-y-1">
                            ...
                            <a href="{{ route('listings.index', ['category' => $grandchild->slug]) }}"
                               class="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-white/80 transition-colors group">
                                <span class="material-symbols-outlined flex-shrink-0" style="font-size:15px; color: {{ $p['icon'] }}90">
                                    {{ $grandchild->icon ?? 'subdirectory_arrow_right' }}
                                </span>
