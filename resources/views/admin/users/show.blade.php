...
<div class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
            ...
            <div class="flex-1">
                <div class="flex items-center gap-3 mb-2">
                    ...
                    @if($user->email_verified_at)
                        <span class="px-3 py-1 text-sm font-medium rounded-full bg-green-100 text-green-800">ایمیل تایید شده</span>
                    @endif
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                    ...
                    <div class="flex items-center gap-2 text-gray-600">
                        ...
                        <span class="text-sm">{{ \App\Services\PersianNumberService::convertToPersian($user->phone) }}</span>
                    </div>
                    @endif
                    <div class="flex items-center gap-2 text-gray-600">
                        <span class="material-symbols-outlined text-[20px]">calendar_today</span>
                        <span class="text-sm">عضویت: {{ \App\Services\JalaliDateService::toJalali($user->created_at, 'Y/m/d') }}</span>
                    </div>
                    @if($user->wallet)
                    <div class="flex items-center gap-2 text-gray-600">
                        <span class="material-symbols-outlined text-[20px]">account_balance_wallet</span>
                        <span class="text-sm font-mono">موجودی: {{ \App\Services\PersianNumberService::convertToPersian(number_format($user->wallet->balance)) }} تومان</span>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl font-bold text-gray-900 mt-1">
                        {{ \App\Services\PersianNumberService::convertToPersian($stats['total_bids']) }}
                    </p>
                </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl font-bold text-gray-900 mt-1">
                        {{ \App\Services\PersianNumberService::convertToPersian($stats['won_auctions']) }}
                    </p>
                </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl font-bold text-gray-900 mt-1">
                        {{ \App\Services\PersianNumberService::convertToPersian($stats['total_orders']) }}
                    </p>
                </div>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-lg font-bold text-gray-900 mt-1">
                        {{ \App\Services\PersianNumberService::convertToPersian(number_format($stats['total_spent'])) }}
                    </p>
                </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl font-bold text-gray-900 mt-1">
                        {{ \App\Services\PersianNumberService::convertToPersian($stats['total_listings']) }}
                    </p>
                </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl font-bold text-gray-900 mt-1">
                        {{ \App\Services\PersianNumberService::convertToPersian($stats['active_listings']) }}
                    </p>
                </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 text-sm text-gray-900">
                            <a href="{{ route('listings.show', $bid->listing) }}" class="text-primary hover:underline">
                                {{ $bid->listing->title }}
                            </a>
                        </td>
                        ...
                        <td class="px-4 py-3 text-sm">
                            @if($bid->listing->current_winner_id === $user->id && $bid->listing->status === 'completed')
                                <span class="px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-800">برنده</span>
                            @elseif($bid->listing->current_winner_id === $user->id)
                        ...
                        <td class="px-4 py-3 text-sm text-gray-500">
                            {{ \App\Services\JalaliDateService::toJalali($bid->created_at, 'Y/m/d H:i') }}
                        </td>
                    </tr>
                    @endforeach
                </tbody>
    ...
    @if($user->role === 'seller' && $user->listings->count() > 0)
    <div class="bg-white rounded-xl border border-gray-200 p-6 mb-6">
        <h3 class="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 text-sm text-gray-900">
                            <a href="{{ route('admin.listings.show', $listing) }}" class="text-primary hover:underline">
                                {{ $listing->title }}
                            </a>
                        </td>
                        <td class="px-4 py-3 text-sm">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 text-sm">
                            <a href="{{ route('admin.orders.show', $order) }}" class="text-primary hover:underline font-mono">
                                #{{ \App\Services\PersianNumberService::convertToPersian($order->id) }}
                            </a>
                        </td>
                        <td class="px-4 py-3 text-sm">
                        ...
                        <td class="px-4 py-3 text-sm font-mono">
                            {{ \App\Services\PersianNumberService::convertToPersian(number_format($order->total)) }} تومان
                        </td>
                        <td class="px-4 py-3 text-sm text-gray-500">
                            {{ \App\Services\JalaliDateService::toJalali($order->created_at, 'Y/m/d') }}
                        </td>
                    </tr>
                    @endforeach
                </tbody>
