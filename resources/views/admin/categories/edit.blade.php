...
<div class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
    <!-- Header -->
    <div class="mb-6">
        <h2 class="text-2xl font-black text-gray-900">ویرایش دسته‌بندی: {{ $category->name }}</h2>
        ...
        <form action="{{ route('admin.categories.update', $category) }}" method="POST">
            ...
            <div class="mt-6">
                ...
                <textarea name="description" 
                          rows="3"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary @error('description') border-red-500 @enderror">{{ old('description', $category->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
