...
@section('title', 'مدیریت کمیسیون دسته‌بندی‌ها')
...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-6xl mx-auto">
        <div class="flex justify-between items-center mb-8">
            ...
            <a href="{{ route('admin.settings.index') }}" class="bg-gray-600 text-white px-6 py-2 rounded-lg hover:bg-gray-700">
                بازگشت به تنظیمات
            </a>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mb-6">
            ...
            <form action="{{ route('admin.category-commissions.store') }}" method="POST" class="space-y-4">
                @csrf
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($categories->filter(fn($c) => $c->categoryCommission) as $category)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 text-sm">
                                    {{ $category->getFullPath() }}
                                </td>
                                <td class="px-6 py-4 text-sm">
                                    @if($category->categoryCommission->type === 'fixed')
                                ...
                                <td class="px-6 py-4 text-sm font-bold">
                                    @if($category->categoryCommission->type === 'fixed')
                                        @price($category->categoryCommission->fixed_amount) تومان
                                    @else
                                        {{ \App\Services\PersianNumberService::convertToPersian($category->categoryCommission->percentage) }}%
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-sm">
                                    <form action="{{ route('admin.category-commissions.destroy', $category->categoryCommission->id) }}" 
                                          method="POST" 
