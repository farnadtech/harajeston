...
<div class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('admin.listings.export', request()->query()) }}" class="px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 flex items-center gap-2 text-sm font-medium">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">
                        {{ \App\Services\PersianNumberService::convertToPersian(\App\Models\Listing::where('status', 'pending')->whereNull('approved_at')->count()) }}
                    </p>
                </div>
        ...
        <form method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div>
                ...
                <select name="status" class="w-full border-gray-300 rounded-lg text-sm focus:ring-primary focus:border-primary">
                    ...
                    <option value="needs_approval" {{ request('status') === 'needs_approval' ? 'selected' : '' }}>نیاز به تایید</option>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            @if($pendingChangesCount > 0)
                                <a href="{{ route('admin.listings.manage', $listing) }}" 
                                   ...
                                   title="مشاهده تغییرات">
                                    ...
                                    {{ \App\Services\PersianNumberService::convertToPersian($pendingChangesCount) }}
                                </a>
                            @else
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex items-center gap-2">
                                ...
                                {{-- Pending + not approved: Show approve and reject buttons --}}
                                @if($listing->status === 'pending' && !$listing->approved_at)
