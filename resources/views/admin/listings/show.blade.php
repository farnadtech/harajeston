...
@section('title', 'جزئیات آگهی - ' . $listing->title)
...
<div class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-4">
            <a href="{{ route('admin.listings.index') }}" class="text-gray-600 hover:text-gray-900">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
                ...
                <div class="flex items-start gap-4 mb-6">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="flex items-center gap-4 text-sm text-gray-500">
                            <span>شناسه: {{ \App\Services\PersianNumberService::convertToPersian($listing->id) }}</span>
                            <span>•</span>
                            <span>{{ \App\Services\JalaliDateService::toJalali($listing->created_at, 'Y/m/d') }}</span>
                        </div>
                    </div>
                </div>
            ...
            <!-- Auction Details -->
            @if($listing->type === 'auction' || $listing->type === 'both')
            <div class="bg-white rounded-xl border border-gray-200 p-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <p class="text-sm font-medium text-gray-900">
                            {{ \App\Services\JalaliDateService::toJalali($listing->starts_at, 'Y/m/d H:i') }}
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-sm font-medium text-gray-900">
                            {{ \App\Services\JalaliDateService::toJalali($listing->ends_at, 'Y/m/d H:i') }}
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-sm font-medium text-gray-900 font-mono">
                            {{ \App\Services\PersianNumberService::convertToPersian(number_format($listing->required_deposit)) }} تومان
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-sm font-medium text-gray-900">
                            {{ \App\Services\PersianNumberService::convertToPersian($listing->participations->count()) }} نفر
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-sm font-medium text-gray-900">
                            {{ $listing->auto_extend ? 'فعال' : 'غیرفعال' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-6">
                ...
                <div class="space-y-2">
                    @foreach($listing->bids->sortByDesc('created_at')->take(10) as $index => $bid)
                    <div class="flex items-center justify-between p-3 {{ $index === 0 ? 'bg-blue-50 border border-blue-100' : 'bg-gray-50' }} rounded-lg">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-full {{ $index === 0 ? 'bg-blue-200' : 'bg-gray-200' }} flex items-center justify-center text-sm font-bold {{ $index === 0 ? 'text-blue-700' : 'text-gray-600' }}">
                                {{ mb_substr($bid->user->name, 0, 2) }}
                            </div>
                            <div>
                                <p class="text-sm font-medium text-gray-900">{{ $bid->user->name }}</p>
                                <p class="text-xs text-gray-500">{{ \App\Services\JalaliDateService::toJalali($bid->created_at, 'Y/m/d H:i') }}</p>
                            </div>
                        </div>
                        <div class="text-left">
                            <p class="text-sm font-bold {{ $index === 0 ? 'text-primary' : 'text-gray-900' }} font-mono">
                                {{ \App\Services\PersianNumberService::convertToPersian(number_format($bid->amount)) }}
                            </p>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-6">
                ...
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        <p class="text-sm font-bold text-gray-900">{{ $listing->seller->name }}</p>
                        <p class="text-xs text-gray-500">{{ $listing->store->name }}</p>
                    </div>
                </div>
                ...
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between">
                        ...
                        <span class="text-gray-900">{{ $listing->seller->email }}</span>
                    </div>
                    @if($listing->seller->phone)
                    <div class="flex justify-between">
                        <span class="text-gray-500">تلفن:</span>
                        <span class="text-gray-900 font-mono">{{ \App\Services\PersianNumberService::convertToPersian($listing->seller->phone) }}</span>
                    </div>
                    @endif
                    <div class="flex justify-between">
                        <span class="text-gray-500">عضویت:</span>
                        <span class="text-gray-900">{{ \App\Services\JalaliDateService::toJalali($listing->seller->created_at, 'Y/m/d') }}</span>
                    </div>
                </div>
                ...
                <div class="space-y-3">
                    <div class="flex justify-between items-center p-3 bg-blue-50 rounded-lg">
                        ...
                        <span class="text-lg font-bold text-blue-600">
                            {{ \App\Services\PersianNumberService::convertToPersian($listing->views) }}
                        </span>
                    </div>
                    ...
                    <div class="flex justify-between items-center p-3 bg-purple-50 rounded-lg">
                        ...
                        <span class="text-lg font-bold text-purple-600">
                            {{ \App\Services\PersianNumberService::convertToPersian($listing->favorites) }}
                        </span>
                    </div>
                    ...
                    <div class="flex justify-between items-center p-3 bg-green-50 rounded-lg">
                        ...
                        <span class="text-lg font-bold text-green-600">
                            {{ \App\Services\PersianNumberService::convertToPersian($listing->shares) }}
                        </span>
                    </div>
                </div>
            </div>
...
function activateListing() {
    if (confirm('آیا از فعال‌سازی این آگهی اطمینان دارید؟')) {
        fetch(`/admin/listings/{{ $listing->id }}/activate`, {
            method: 'POST',
            headers: {
...
function suspendListing() {
    ...
    if (reason) {
        fetch(`/admin/listings/{{ $listing->id }}/suspend`, {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Accept': 'application/json',
