...
<div class="container mx-auto px-4 py-6 max-w-2xl">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <form action="{{ route('admin.category-attributes.update', [$category, $attribute]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select name="type" id="attributeType" required
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                        <option value="select" {{ old('type', $attribute->type) === 'select' ? 'selected' : '' }}>انتخابی (لیست کشویی)</option>
                        <option value="text" {{ old('type', $attribute->type) === 'text' ? 'selected' : '' }}>متنی</option>
                        <option value="number" {{ old('type', $attribute->type) === 'number' ? 'selected' : '' }}>عددی</option>
                    </select>
                    @error('type')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div id="optionsField" style="display: {{ old('type', $attribute->type) === 'select' ? 'block' : 'none' }}">
                    <label class="block text-sm font-medium text-gray-700 mb-2">گزینه‌ها (با کاما جدا کنید)</label>
                    <textarea name="options" rows="3"
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent"
                              placeholder="مثال: 4GB, 8GB, 16GB, 32GB">{{ old('options', $attribute->options ? implode(', ', $attribute->options) : '') }}</textarea>
                ...
                <div>
                    ...
                    <input type="number" name="order" value="{{ old('order', $attribute->order) }}" min="0"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                    @error('order')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
