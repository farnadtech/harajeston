...
@section('title', 'تنظیمات عمومی سایت')
...
<div class="container mx-auto px-4 py-8 max-w-4xl">
    ...
    <form method="POST" action="{{ route('admin.settings.general.update') }}" id="generalForm">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="site_description" rows="2"
                              class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                              maxlength="500">{{ $settings['site_description'] }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">آیکون پیش‌فرض (Material Icon)</label>
                    <div class="flex gap-2">
                        <input type="text" name="site_icon" id="siteIconInput" value="{{ $settings['site_icon'] }}"
                        ...
                        <div class="w-12 h-12 bg-blue-50 rounded-lg flex items-center justify-center border border-blue-200">
                            <span class="material-symbols-outlined text-blue-600 text-2xl" id="iconPreview">{{ $settings['site_icon'] ?: 'gavel' }}</span>
                        </div>
                    </div>
                ...
                <div>
                    ...
                    <div class="border-2 border-dashed border-gray-300 rounded-xl p-4 text-center hover:border-blue-400 transition-colors cursor-pointer" id="logoDropzone">
                        @if($settings['site_logo'])
                            <img src="{{ rtrim(config('app.url'), '/') . '/storage/' . $settings['site_logo'] }}" alt="لوگو" class="h-16 mx-auto mb-2 object-contain" id="logoPreview">
                        @else
                ...
                <div>
                    ...
                    <div class="border-2 border-dashed border-gray-300 rounded-xl p-4 text-center hover:border-blue-400 transition-colors cursor-pointer" id="faviconDropzone">
                        @if($settings['site_favicon'])
                            <img src="{{ rtrim(config('app.url'), '/') . '/storage/' . $settings['site_favicon'] }}" alt="فاویکون" class="h-16 mx-auto mb-2 object-contain" id="faviconPreview">
                        @else
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-5">
                <div>
                    ...
                    <div class="flex gap-2 items-center">
                        <input type="color" name="color_primary" id="colorPrimary" value="{{ $settings['color_primary'] }}"
                               class="w-12 h-10 rounded cursor-pointer border border-gray-300">
                        <input type="text" id="colorPrimaryText" value="{{ $settings['color_primary'] }}"
                ...
                <div>
                    ...
                    <div class="flex gap-2 items-center">
                        <input type="color" name="color_primary_hover" id="colorPrimaryHover" value="{{ $settings['color_primary_hover'] }}"
                               class="w-12 h-10 rounded cursor-pointer border border-gray-300">
                        <input type="text" id="colorPrimaryHoverText" value="{{ $settings['color_primary_hover'] }}"
                               class="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm font-mono"
                               pattern="^#[0-9a-fA-F]{6}$" maxlength="7">
                    </div>
                ...
                <div>
                    ...
                    <div class="flex gap-2 items-center">
                        <input type="color" name="color_secondary" id="colorSecondary" value="{{ $settings['color_secondary'] }}"
                               class="w-12 h-10 rounded cursor-pointer border border-gray-300">
                        <input type="text" id="colorSecondaryText" value="{{ $settings['color_secondary'] }}"
                               class="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm font-mono"
                               pattern="^#[0-9a-fA-F]{6}$" maxlength="7">
                    </div>
                ...
                <div>
                    ...
                    <div class="flex gap-2 items-center">
                        <input type="color" name="color_bg" id="colorBg" value="{{ $settings['color_bg'] }}"
                               class="w-12 h-10 rounded cursor-pointer border border-gray-300">
                        <input type="text" id="colorBgText" value="{{ $settings['color_bg'] }}"
                               class="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm font-mono"
                               pattern="^#[0-9a-fA-F]{6}$" maxlength="7">
                    </div>
                ...
                <div>
                    ...
                    <div class="flex gap-2 items-center">
                        <input type="color" name="color_text" id="colorText" value="{{ $settings['color_text'] }}"
                               class="w-12 h-10 rounded cursor-pointer border border-gray-300">
                        <input type="text" id="colorTextText" value="{{ $settings['color_text'] }}"
                               class="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm font-mono"
                               pattern="^#[0-9a-fA-F]{6}$" maxlength="7">
                    </div>
            ...
            <div class="mt-6 p-4 rounded-xl border border-gray-200 bg-gray-50">
                ...
                <div class="flex flex-wrap gap-3 items-center">
                    <button type="button" id="previewBtn" class="px-5 py-2 rounded-lg text-white text-sm font-bold transition-colors" style="background-color: {{ $settings['color_primary'] }}">دکمه اصلی</button>
                    <button type="button" id="previewBtnSecondary" class="px-5 py-2 rounded-lg text-white text-sm font-bold" style="background-color: {{ $settings['color_secondary'] }}">دکمه ثانویه</button>
                    <span id="previewText" class="text-sm font-bold" style="color: {{ $settings['color_primary'] }}">لینک و متن رنگی</span>
                    <div id="previewBg" class="px-4 py-2 rounded-lg text-sm" style="background-color: {{ $settings['color_bg'] }}; color: {{ $settings['color_text'] }}">پس‌زمینه و متن</div>
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-end gap-3">
            <a href="{{ route('admin.settings.index') }}" class="px-6 py-2.5 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors">
                بازگشت
            </a>
...
function uploadFile(file, type, dropzoneId) {
    ...
    fetch('{{ route("admin.settings.general.upload-logo") }}', { method: 'POST', body: formData })
        .then(function(r) {
            if (!r.ok) {
