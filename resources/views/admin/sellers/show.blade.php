    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-lg border border-gray-200 p-6">
                ...
                @if($seller->seller_rejection_reason)
                    <div class="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
                        <p class="text-sm font-bold text-red-900 mb-1">دلیل رد/تعلیق:</p>
                        <p class="text-sm text-red-800">{{ $seller->seller_rejection_reason }}</p>
                    </div>
                @endif
            ...
            <div class="bg-white rounded-lg border border-gray-200 p-6">
                ...
                @if($seller->seller_request_data)
                    <div class="space-y-3">
                        <div class="flex justify-between py-2 border-b">
                            <span class="text-gray-600">نام فروشگاه:</span>
                            <span class="font-bold">{{ $seller->seller_request_data['store_name'] ?? '-' }}</span>
                        </div>
                        <div class="flex justify-between py-2 border-b">
                            <span class="text-gray-600">شماره تماس:</span>
                            <span class="font-bold">{{ $seller->seller_request_data['phone'] ?? '-' }}</span>
                        </div>
                        <div class="flex justify-between py-2 border-b">
                            <span class="text-gray-600">کد ملی:</span>
                            <span class="font-bold">{{ $seller->seller_request_data['national_id'] ?? '-' }}</span>
                        </div>
                        <div class="flex justify-between py-2 border-b">
                            <span class="text-gray-600">بانک:</span>
                            <span class="font-bold">{{ $seller->seller_request_data['bank_name'] ?? '-' }}</span>
                        </div>
                        <div class="flex justify-between py-2 border-b">
                            <span class="text-gray-600">شماره حساب:</span>
                            <span class="font-bold text-sm">{{ $seller->seller_request_data['bank_account'] ?? '-' }}</span>
                        </div>
                        @if(isset($seller->seller_request_data['store_description']))
                        <div class="py-2">
                            <span class="text-gray-600 block mb-2">توضیحات فروشگاه:</span>
                            <p class="text-sm text-gray-700 bg-gray-50 p-3 rounded">
                                {{ $seller->seller_request_data['store_description'] }}
                            </p>
                        </div>
                        @endif
                        @if(isset($seller->seller_request_data['address']))
                        <div class="py-2">
                            <span class="text-gray-600 block mb-2">آدرس:</span>
                            <p class="text-sm text-gray-700 bg-gray-50 p-3 rounded">
                                {{ $seller->seller_request_data['address'] }}
                            </p>
                        </div>
                        @endif
                    </div>
            ...
            <div class="bg-white rounded-lg border border-gray-200 p-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <p class="text-2xl font-black text-gray-900">@persian($stats['total_listings'])</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">آگهی‌های فعال</p>
                        <p class="text-2xl font-black text-green-600">@persian($stats['active_listings'])</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">فروش موفق</p>
                        <p class="text-2xl font-black text-blue-600">@persian($stats['completed_sales'])</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">درآمد کل</p>
                        <p class="text-xl font-black text-primary">@persian(number_format($stats['total_revenue'])) تومان</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg border border-gray-200 p-6">
                ...
                <div class="space-y-3 text-sm">
                    <div>
                        ...
                        <span class="font-bold">@persian($seller->id)</span>
                    </div>
                    <div>
                        <span class="text-gray-600">ایمیل:</span>
                        <span class="font-bold">{{ $seller->email }}</span>
                    </div>
                    <div>
                        <span class="text-gray-600">تاریخ ثبت‌نام:</span>
                        <span class="font-bold">{{ \Morilog\Jalali\Jalalian::fromDateTime($seller->created_at)->format('Y/m/d') }}</span>
                    </div>
                    <div>
                        <span class="text-gray-600">تاریخ درخواست:</span>
                        <span class="font-bold">{{ \Morilog\Jalali\Jalalian::fromDateTime($seller->seller_requested_at)->format('Y/m/d') }}</span>
                    </div>
                    @if($seller->seller_approved_at)
                    <div>
                        <span class="text-gray-600">تاریخ تایید:</span>
                        <span class="font-bold">{{ \Morilog\Jalali\Jalalian::fromDateTime($seller->seller_approved_at)->format('Y/m/d') }}</span>
                    </div>
                    @endif
                </div>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3">
                                ...
                                <div class="text-xs text-gray-500">شناسه: @persian($listing->id)</div>
                            </td>
                            <td class="px-4 py-3 text-sm text-gray-600">
                                {{ $listing->category->name ?? '-' }}
                            </td>
                            <td class="px-4 py-3 text-sm font-bold text-gray-900">
                                @persian(number_format($listing->starting_price)) تومان
                            </td>
                            <td class="px-4 py-3">
                                ...
                                @else
                                    <span class="px-2 py-1 text-xs font-medium rounded-full bg-gray-100 text-gray-800">{{ $listing->status }}</span>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-xs text-gray-500">
                                {{ \Morilog\Jalali\Jalalian::fromDateTime($listing->created_at)->format('Y/m/d') }}
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    ...
                                    @if($listing->status === 'active')
                                        <button onclick="showListingSuspendModal({{ $listing->id }})" 
                                                class="text-red-600 hover:text-red-800" 
                                                title="تعلیق">
                                            <span class="material-symbols-outlined text-[18px]">block</span>
                                        </button>
                                    @elseif($listing->status === 'suspended')
                                        <button onclick="showListingActivateModal({{ $listing->id }})" 
...
function showListingSuspendModal(listingId) {
    ...
    form.action = '{{ url("/admin/listings") }}/' + listingId + '/suspend';
...
function showListingActivateModal(listingId) {
    ...
    form.action = '{{ url("/admin/listings") }}/' + listingId + '/activate';
