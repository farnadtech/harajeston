...
<div dir="rtl">
    ...
    {{-- Layout اصلی: sidebar ابزار + canvas --}}
    <div style="display:flex; gap:20px; align-items:flex-start;">
        ...
        <div style="width:240px; flex-shrink:0; display:flex; flex-direction:column; gap:16px;">
            ...
            <div style="background:white; border:1px solid #e5e7eb; border-radius:12px; padding:16px;">
                ...
                <form action="{{ route('admin.homepage.card-style') }}" method="POST">
                    @csrf
                    @foreach(['classic'=>'کلاسیک','modern'=>'مدرن','minimal'=>'مینیمال','horizontal'=>'افقی'] as $val=>$lbl)
                    <label style="display:flex; align-items:center; gap:10px; padding:8px 10px; border-radius:8px; border:1px solid {{ $cardStyle === $val ? '#3b82f6' : '#e5e7eb' }}; background:{{ $cardStyle === $val ? '#eff6ff' : 'white' }}; margin-bottom:6px; cursor:pointer;">
                        <input type="radio" name="card_style" value="{{ $val }}"
                               {{ $cardStyle === $val ? 'checked' : '' }}
                               style="accent-color:#3b82f6;" onchange="this.form.submit()">
                        <span style="font-size:13px; font-weight:{{ $cardStyle === $val ? '600' : '400' }}; color:{{ $cardStyle === $val ? '#3b82f6' : '#374151' }};">{{ $lbl }}</span>
                        @if($cardStyle === $val)
            ...
            <div style="background:white; border:1px solid #e5e7eb; border-radius:12px; padding:16px;">
                ...
                <button onclick="addBlock('{{ $type }}')"
                    ...
                    <span class="material-symbols-outlined" style="font-size:16px; color:#9ca3af;">{{ $info['icon'] }}</span>
            ...
            <div style="background:#f3f4f6; border-radius:12px; padding:16px; min-height:500px;">
                ...
                <div id="blocks-canvas" style="display:flex; flex-direction:column; gap:8px;">
                    @forelse($blocks as $block)
                        @include('admin.homepage.partials.block-item', ['block' => $block])
                    @empty
...
{{-- ذخیره configs در JS --}}
<script id="blocks-data" type="application/json">@json($blocks)</script>

@push('scripts')
<script src="https://cdn.jsdelivr.net/npm/sortablejs@1.15.0/Sortable.min.js"></script>
<script>
const CSRF = '{{ csrf_token() }}';
const SAVE_URL   = '{{ route('admin.homepage.blocks.save') }}';
const ADD_URL    = '{{ route('admin.homepage.blocks.add') }}';
const BASE_URL   = '{{ url('admin/homepage/blocks') }}';
...
function buildEditForm(id, type, config) {
    ...
    if (type === 'hero') {
        ...
        <div style="display:flex; flex-direction:column; gap:16px;">
            ...
            <div id="hero-image-section" style="display:${mode==='image'?'flex':'none'}; flex-direction:column; gap:12px;">
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
                    ...
                    <div>
                        ...
                        <input type="text" name="config[button_url]" value="${esc(config.button_url||'')}" class="input" placeholder="{{ route('listings.index') }}">
                    </div>
                </div>
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
                    <div>
            ...
            {{-- تنظیمات بنرهای کناری (مشترک برای هر دو حالت) --}}
        ...
        const allCats = @json(\App\Models\Category::whereNull('parent_id')->orderBy('name')->with(['children' => fn($q) => $q->orderBy('name')->with(['children' => fn($q2) => $q2->orderBy('name')])])->get(['id','name','parent_id']));
        ...
        const renderBannerRows = (items) => items.map((b,i) => `
            <div class="banner-row p-3 bg-gray-50 rounded-lg border border-gray-100 mb-3" data-index="${i}">
                ...
                <div>
                    ...
                    ${b.custom_image ? `<img src="{{ url('storage') }}/${b.custom_image}" style="width:100%; height:80px; object-fit:cover; border-radius:6px; margin-bottom:6px;">
