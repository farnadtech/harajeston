...
@section('title', 'مدیریت نظرات فروشندگان')
@section('header-title', 'مدیریت نظرات فروشندگان')
@section('header-subtitle', 'تایید، رد یا حذف نظرات خریداران درباره فروشندگان')
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
        <form method="GET" class="flex flex-wrap gap-4">
            ...
            @if(request()->has('status'))
                <a href="{{ route('admin.seller-reviews.index') }}" class="px-6 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors">
                    حذف فیلترها
                </a>
            @endif
        </form>
            ...
            <div class="p-6 border-b border-gray-100 hover:bg-gray-50 transition-colors">
                <div class="flex items-start justify-between gap-4 mb-4">
                    <div class="flex-1">
                        <div class="flex items-center gap-3 mb-2">
                            ...
                            <div class="flex items-center gap-1 mr-2">
                                ...
                                @endfor
                                <span class="text-xs text-gray-600 mr-1">({{ $review->rating }})</span>
                            </div>
                        ...
                        <a href="{{ route('admin.orders.show', $review->order_id) }}" class="text-sm text-primary hover:underline mb-2 block">
                            سفارش #{{ $review->order_id }}
                        </a>
                    ...
                    <div class="flex gap-2">
                        @if($review->status === 'pending')
                            <form method="POST" action="{{ route('admin.seller-reviews.approve', $review->id) }}" class="inline">
                                @csrf
                            ...
                            <form method="POST" action="{{ route('admin.seller-reviews.reject', $review->id) }}" class="inline">
                                @csrf
                        ...
                        <form method="POST" action="{{ route('admin.seller-reviews.destroy', $review->id) }}" class="inline" onsubmit="return confirm('آیا مطمئن هستید؟')">
                            @csrf
                            @method('DELETE')
