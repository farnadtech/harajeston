...
<main class="flex-grow w-full max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
    ...
    <?php if(($listing->hasEnded() || in_array($listing->status, ['ended', 'completed'])) && $listing->status !== 'suspended'): ?>
        <?php
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-green-700 text-sm">
                            نحوه اتمام: <strong><?php echo e($isBuyNow ? 'خرید فوری' : 'بالاترین پیشنهاد'); ?></strong>
                        </p>
                    ...
                    ?>
                    <?php if($hasOrder): ?>
                        <a href="<?php echo e(route('orders.index')); ?>" class="px-4 py-2 bg-green-600 text-white text-sm font-bold rounded-xl hover:bg-green-700 transition-colors flex items-center gap-1 flex-shrink-0">
                            <span class="material-symbols-outlined text-sm">shopping_bag</span>
                            سفارشات من
                        </a>
                    <?php else: ?>
                        <a href="<?php echo e(route('checkout.auction', $listing)); ?>" class="px-4 py-2 bg-green-600 text-white text-sm font-bold rounded-xl hover:bg-green-700 transition-colors flex items-center gap-1 flex-shrink-0">
        ...
        <?php else: ?>
            ...
            <div class="bg-gray-50 border-2 border-gray-200 rounded-xl p-6">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 <?php echo e($winner ? 'bg-yellow-100' : 'bg-gray-100'); ?> rounded-full flex items-center justify-center flex-shrink-0">
                        <span class="material-symbols-outlined <?php echo e($winner ? 'text-yellow-600' : 'text-gray-600'); ?> text-2xl"><?php echo e($winner ? 'emoji_events' : 'event_busy'); ?></span>
                    </div>
                    <div class="flex-1">
                        <?php if($isProcessing): ?>
                        ...
                        ?>
                        <p class="text-gray-700 text-sm mt-1">🏆 برنده: <strong class="text-yellow-700" dir="ltr"><?php echo e($maskedPhone); ?></strong></p>
                    ...
                    <div class="bg-gradient-to-br from-orange-50 to-red-50 rounded-xl border border-orange-200 p-4">
                        ...
                        <?php if(auth()->guard()->check()): ?>
                            <?php if(auth()->user()->role !== 'admin' && $listing->seller_id !== auth()->id()): ?>
                                <form id="buyNowForm" action="<?php echo e(route('listings.participate', $listing)); ?>" method="POST">
                                    <?php echo csrf_field(); ?>
...
$__html = app('livewire')->mount($__name, $__params, 'ZwTDKLe', $__slots ?? [], get_defined_vars());
...
?>
                    ...
                    <div class="flex justify-between items-end mb-3">
                        <div>
                            ...
                            <div class="flex items-baseline gap-1">
                                <span class="text-3xl font-black text-primary">
                                    ...
                                    ?>
                                    <?php echo app(\App\Services\PersianNumberService::class)->formatNumber($currentPrice, true); ?>
                                </span>
                                <span class="text-gray-500 font-medium">تومان</span>
                            </div>
                            <?php if($isUserWinning && $listing->status === 'active'): ?>
                        ...
                        <div class="text-left">
                            ...
                            <span class="font-bold text-gray-800 text-lg"><?php echo app(\App\Services\PersianNumberService::class)->toPersian($listing->bids->count()); ?> پیشنهاد</span>
                        </div>
                    </div>
                ...
                <?php if(auth()->guard()->check()): ?>
                    ...
                    <?php elseif(auth()->check() && $listing->seller_id === auth()->id()): ?>
                        ...
                        <?php if(in_array($listing->status, ['ended', 'completed'])): ?>
                            <?php
                            ...
                            <div class="p-6 bg-blue-50 rounded-xl border-2 border-blue-200 text-center">
                                ...
                                <?php if($listing->current_winner_id): ?>
                                    <?php $winner = \App\Models\User::find($listing->current_winner_id); ?>
                                    <p class="text-sm text-blue-700 mb-4">
                                        برنده: <strong><?php echo e($winner?->name ?? 'کاربر'); ?></strong>
                                ...
                                <?php endif; ?>
                                <?php if($order): ?>
                                    <a href="<?php echo e(route('orders.show', $order)); ?>" class="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-colors">
                                        <span class="material-symbols-outlined">shopping_bag</span>
                                        مشاهده سفارش
                                    </a>
                                <?php else: ?>
                                    <a href="<?php echo e(route('orders.index')); ?>" class="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-colors">
                                        <span class="material-symbols-outlined">shopping_bag</span>
                                        مشاهده سفارشات
                                    </a>
                                <?php endif; ?>
...
unset($__errorArgs, $__bag); ?>
    ...
    <!-- بنر برنده شدن -->
    <?php if($listing->status === 'ended' && auth()->check() && $listing->current_winner_id === auth()->id()): ?>
                ...
                <div class="flex-1">
                    ...
                    <div class="bg-white rounded-lg p-4 mb-4 space-y-2">
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="font-bold text-gray-900"><?php echo e(\App\Services\PersianNumberService::convertToPersian(number_format($totalAmount))); ?> تومان</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-600">سپرده پرداخت شده:</span>
                            <span class="font-bold text-green-600"><?php echo e(\App\Services\PersianNumberService::convertToPersian(number_format($depositAmount))); ?> تومان</span>
                        </div>
                        <div class="border-t border-gray-200 pt-2 flex justify-between">
                            <span class="text-gray-900 font-medium">مبلغ باقیمانده:</span>
                            <span class="font-black text-primary text-lg"><?php echo e(\App\Services\PersianNumberService::convertToPersian(number_format($remainingAmount))); ?> تومان</span>
                        </div>
                    </div>
                    <p class="text-green-800 text-sm mb-4">
                        برای تکمیل خرید، آدرس و روش ارسال خود را انتخاب کنید.
                        مهلت پرداخت: <span class="font-bold"><?php echo e(\App\Services\PersianNumberService::convertToPersian(\App\Services\JalaliDateService::toJalali($listing->finalization_deadline, 'Y/m/d H:i'))); ?></span>
                        (<?php echo e(\App\Services\PersianNumberService::convertToPersian($deadlineHours)); ?> ساعت)
                    </p>
                    <a href="<?php echo e(route('checkout.auction', $listing)); ?>" class="inline-flex items-center gap-2 px-6 py-3 bg-green-600 hover:bg-green-700 text-white rounded-xl font-bold transition-colors">
...
<?php if($listing->buy_now_price && $listing->buy_now_price > 0 && $listing->isActive() && auth()->check()): ?>
<?php
...
<div id="buyNowModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-[9999] hidden items-center justify-center p-4" onclick="if(event.target === this) closeBuyNowModal()">
    <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] flex flex-col overflow-hidden transform transition-all" onclick="event.stopPropagation()">
        ...
        <div class="p-6 overflow-y-auto flex-1">
            ...
            <?php if($shippingMethods->isNotEmpty()): ?>
            <div class="mb-6">
                ...
                <div class="space-y-2">
                    ...
                    <label class="flex items-center gap-3 p-3 border-2 border-gray-200 rounded-xl cursor-pointer hover:border-orange-400 transition-colors shipping-method-option">
                        <input type="radio" name="shipping_method" value="<?php echo e($method->id); ?>" 
                               data-cost="<?php echo e($finalCost); ?>" 
                               class="w-5 h-5 text-orange-500 shipping-method-radio"
                               onchange="updateBuyNowTotal()"
                               <?php echo e($loop->first ? 'checked' : ''); ?>>
                        <div class="flex-1">
                            <div class="font-bold text-gray-900"><?php echo e($method->name); ?></div>
                            <?php if($method->description): ?>
                            <div class="text-xs text-gray-600 mt-1"><?php echo e($method->description); ?></div>
                            <?php endif; ?>
                        </div>
                        <div class="font-bold text-orange-600"><?php echo app(\App\Services\PersianNumberService::class)->formatNumber($finalCost, true); ?> تومان</div>
                    </label>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
            </div>
            ...
            <div class="space-y-3 mb-6">
                <div class="flex justify-between items-center py-2 border-b border-gray-200">
                    ...
                    <span class="font-bold text-gray-900"><?php echo app(\App\Services\PersianNumberService::class)->formatNumber($listing->buy_now_price, true); ?> تومان</span>
                </div>
                ...
                <div class="flex justify-between items-center py-2 border-b border-gray-200">
                    ...
                    <span class="font-bold text-gray-900" id="shippingCostDisplay"><?php echo app(\App\Services\PersianNumberService::class)->formatNumber($firstMethodCost, true); ?> تومان</span>
                </div>
                <?php endif; ?>
                ...
                <div class="flex justify-between items-center py-2 border-b border-gray-200">
                    <span class="text-gray-600">سپرده پرداخت شده:</span>
                    <span class="font-bold text-green-600"><?php echo app(\App\Services\PersianNumberService::class)->formatNumber($depositPaid, true); ?> تومان</span>
                </div>
                ...
                <div class="flex justify-between items-center py-3 bg-orange-50 rounded-lg px-3">
                    ...
                    <span class="font-black text-orange-600 text-xl" id="totalAmountDisplay"><?php echo app(\App\Services\PersianNumberService::class)->formatNumber($amountToPay + ($shippingMethods->isNotEmpty() ? $firstMethodCost : 0), true); ?> تومان</span>
                </div>
...
<script>
const buyNowBaseAmount = <?php echo e($amountToPay); ?>;
const hasDeposit = <?php echo e($userParticipation ? 'true' : 'false'); ?>;
        ...
        <div class="p-6">
            <div class="bg-gray-50 rounded-xl p-4 mb-6">
                <div class="flex items-start gap-3">
                    ...
                    <div class="text-sm text-gray-700 leading-relaxed">
                        با تایید این عملیات، مبلغ <span class="font-bold text-primary"><?php echo e(\App\Services\PersianNumberService::convertToPersian(number_format($remainingAmount))); ?> تومان</span> از کیف پول شما کسر شده و سفارش شما ثبت خواهد شد.
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-3 mb-6">
                ...
                <div class="flex justify-between items-center py-2">
                    <span class="text-gray-900 font-medium">مبلغ باقیمانده:</span>
                    <span class="font-black text-primary text-xl"><?php echo e(\App\Services\PersianNumberService::convertToPersian(number_format($remainingAmount))); ?> تومان</span>
                </div>
            </div>
