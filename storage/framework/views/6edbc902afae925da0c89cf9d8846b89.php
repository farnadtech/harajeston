...
<div class="bg-white rounded-2xl border border-gray-200 overflow-hidden hover:shadow-xl transition-all duration-300 group relative">
    ...
    <?php elseif($listing->status === 'ended' || $listing->status === 'completed'): ?>
        <div class="absolute top-3 right-3 z-10">
            <span class="px-3 py-1.5 bg-gray-600 text-white text-xs font-bold rounded-full shadow-lg flex items-center gap-1">
    ...
    <?php if($listing->buy_now_price && !in_array($listing->status, ['ended', 'completed'])): ?>
    ...
    <div class="p-4">
        ...
        <?php
            // Get the latest bid to show current price
            $latestBid = $listing->bids()->orderBy('amount', 'desc')->first();
            $displayPrice = $latestBid ? $latestBid->amount : $listing->starting_price;
        ?>
